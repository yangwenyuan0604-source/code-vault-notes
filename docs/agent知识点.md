...
| 话语 | 含义 | 语境示例 |
|------|------|---------|
| **Agentic** | 具有自主性的、能自己拿主意干活的 | "This is an agentic workflow, not just a chatbot" |
| **Agentic AI** | 能自主规划+执行+反思的 AI 系统 | 区别于 Generative AI（纯生成内容） |
| **Agentic RAG** | Agent 主导检索策略的 RAG（Agent 决定何时查、查什么、怎么查） | 区别于 Naive RAG（一次 query→search→answer） |
| **Agent-native** | 从设计之初就以 Agent 范式构建的 | "LangGraph is an agent-native framework" |
| **Fully autonomous** | 完全自主，无需人插手 | "We're not at fully autonomous yet, still HITL" |

#### 系统与架构

| 话语 | 含义 |
|------|------|
| **Cognitive Architecture** | Agent 的"思维架构" — 记忆、推理、规划如何组织。类比人脑的认知结构 |
| **Agent Loop / Agentic Loop** | Agent 核心循环：感知→思考→行动→观察→反思→循环 |
| **Orchestration** | 编排。协调多个 Agent/工具按流程工作。不同于 Choreography（去中心化协作） |
| **Agent Swarm** | 蜂群式多 Agent，大量简单 Agent 涌现复杂行为 |
| **Agent Mesh** | Agent 网状拓扑，每个 Agent 可以跟任意其他 Agent 通信 |
| **Agent Graph** | 用有向图（DAG/状态图）定义 Agent 决策流程。LangGraph 的核心概念 |
| **Multi-Agent System (MAS)** | 多 Agent 系统。≥2 个 Agent 协作完成复杂任务 |
| **Agent-to-Agent (A2A)** | Agent 间通信协议/范式 |
| **Agent-as-a-Service (AaaS)** | 把 Agent 能力包装成 API 服务对外提供 |

#### 控制与安全

| 话语 | 含义 |
|------|------|
| **Human-in-the-Loop (HITL)** | 关键时刻人介入审批。退款>100元→人工确认→继续。Agent 安全核心机制 |
| **Human-on-the-Loop (HOTL)** | 人不介入每步，但旁路监控，异常时接管 |
| **Guardrails** | 安全护栏：输入过滤、工具限权、输出审核三层 |
| **Alignment** | 对齐。Agent 行为符合人类意图和价值观 |
| **Jailbreak** | 越狱。用户用 Prompt 技巧绕过 Agent 的安全限制 |
| **Grounding** | 落地/接地。让 Agent 的回答基于真实数据，而非编造。RAG 是 Grounding 主要手段 |
| **Hallucination** | 幻觉。Agent 生成看似合理但事实上错误的内容 |
| **Constitutional AI** | 用一套"宪法规则"约束 Agent 行为，Claude 的核心训练方法 |

#### 工具与能力

| 话语 | 含义 |
|------|------|
| **Tool Use / Function Calling** | Agent 调用外部工具/API 的能力。Agent 区别于 Chatbot 的核心 |
| **MCP (Model Context Protocol)** | Anthropic 提出的 Agent-工具标准化协议。类比 USB-C 之于外设 |
| **A2A Protocol** | Google 提出的 Agent-Agent 通信协议 |
| **Tool Description Engineering** | 工具描述工程。写好工具的描述文字，LLM 才能正确选择工具 |
| **Code Execution** | Agent 能写代码、运行代码、看结果、改代码。Code Interpreter 的核心 |
| **Browser Use** | Agent 能操作浏览器（点击、输入、滚动、读取页面） |
| **Computer Use** | Agent 能像人一样操作电脑（鼠标、键盘、屏幕理解） |

#### 记忆与上下文

| 话语                      | 含义                                                          |
| ----------------------- | ----------------------------------------------------------- |
| **Context Window**      | 模型一次能"看到"的最大 token 数。Agent 的"视野"上限                          |
| **Short-term Memory**   | 短期记忆 = 当前对话历史                                               |
| **Long-term Memory**    | 长期记忆 = 跨会话持久化存储（向量 DB、知识图谱）                                 |
| **Working Memory**      | 工作记忆 = 当前任务中间结果的暂存区（scratchpad）                             |
| **Episodic Memory**     | 情景记忆 = 记得"上次跟这个用户聊了什么"                                      |
| **Semantic Memory**     | 语义记忆 = 记得"Java 单例有几种写法"这类知识                                 |
| **Context Engineering** | 上下文工程。系统化管理"塞进 Context Window 的内容"，比 Prompt Engineering 更底层 |
| **Token Budget**        | 给每次请求分配 token 预算：系统提示占多少、工具定义占多少、历史占多少、留给输出多少               |

#### 开发与运维

| 话语 | 含义 |
|------|------|
| **AgentOps** | Agent 的 DevOps：可观测性、监控、日志、灰度、回滚 |
| **Eval / Evaluation** | Agent 评测。量化 Agent 表现：任务完成率、工具调用准确率、回答质量 |
| **Tracing** | 链路追踪。一次 Agent 调用经过哪些步骤、调了哪些工具、每步耗时 |
| **Prompt Versioning** | Prompt 版本管理。System Prompt 改了之后要像代码一样做 diff、回滚、AB 测试 |
| **LLM-as-a-Judge** | 用 LLM 当裁判评估另一个 LLM 的输出。Agent 评测常用手段 |
| **Fallback** | 兜底策略。Agent 失败时的降级方案（如切换便宜模型、转人工） |
| **Rate Limiting** | 速率限制。控制 Agent 调用外部 API 的频率，防止打爆或被封 |

#### 范式和设计哲学

| 话语 | 含义 |
|------|------|
| **Co-pilot vs Agent** | Copilot = 辅助人干活（GitHub Copilot）；Agent = 替人干活（Devin）。区别在于自主程度 |
| **Agent-first Design** | 以 Agent 为中心的设计：先想 Agent 能自主做什么，再考虑 UI/API 怎么配合 |
| **Composability** | 可组合性。Agent 可以像乐高一样自由组合（一个Agent调另一个Agent） |
| **Declarative vs Imperative** | 声明式（描述想要什么结果）vs 命令式（逐步指挥怎么做）。Agent 偏向声明式 |
| **Reasoning vs Acting** | ReAct 的两个核心：推理（想清楚怎么做）和执行（真的去做） |
| **Step-by-step Reasoning** | 逐步推理。Chain-of-Thought (CoT) 的核心：一步步想比直接给答案准确率高 |
| **Tree of Thoughts (ToT)** | 思维树。同时探索多条推理路径，选最优的一条。比 CoT 更高级 |
| **ReAct** | Reasoning + Acting。Agent 主流范式：边想边做 |
| **Plan-and-Execute** | 先完整规划，再逐步执行。不同于 ReAct 的"走一步看一步" |
| **Reflection / Self-Correction** | 自我反思/自我纠错。Agent 执行后评估自己的输出，不满意就重来 |

#### 业界关键概念

| 话语 | 含义 |
|------|------|
| **AGI (Artificial General Intelligence)** | 通用人工智能。能做任何人类能做的智力任务 |
| **Agent Platform** | Agent 平台。提供 Agent 开发/部署/监控一站式服务（Dify, Coze, LangSmith） |
| **Model Router** | 模型路由。简单任务→便宜模型，复杂任务→强模型。降本核心手段 |
| **Prompt Injection** | 提示注入攻击。用特殊构造的输入劫持 Agent 行为 |
| **SOP-driven Agent** | 用标准作业流程（SOP）驱动 Agent 行为。MetaGPT 的核心思想 |
| **Role-playing Agent** | 角色扮演 Agent。让 Agent 扮演 PM/程序员/测试等角色协作 |
| **Embodied Agent** | 具身 Agent。有"身体"的 Agent（机器人、智能家居），能感知物理世界并行动 |
...
Agent                ≈ 智能体 ≈ AI Agent ≈ Autonomous Agent ≈ 自主智能体
Tool Calling         ≈ Function Calling ≈ Tool Use ≈ 函数调用 ≈ 工具调用
Agent Loop           ≈ Agentic Loop ≈ Reasoning Loop ≈ 思考-行动循环
RAG                  ≈ 检索增强生成 ≈ Grounded Generation ≈ 基于知识库的回答
Multi-Agent          ≈ 多智能体 ≈ Agent Swarm ≈ Agent Team ≈ MAS
Guardrails           ≈ 护栏 ≈ 安全围栏 ≈ Safety Guard ≈ 安全策略
Orchestration        ≈ 编排 ≈ Agent Orchestration ≈ 工作流编排
Prompt Engineering   ≈ 提示工程 ≈ 提示词工程 ≈ Prompt 设计
Context Window       ≈ 上下文窗口 ≈ Context Length ≈ 上下文长度
System Prompt        ≈ 系统提示 ≈ System Message ≈ 角色设定 ≈ 人设 Prompt
HITL                 ≈ 人在回路 ≈ 人工审核 ≈ 人机协同
```
